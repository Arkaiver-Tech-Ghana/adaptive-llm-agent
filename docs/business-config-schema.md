...
| `tool_provider` | string | `"in_memory"` | wired | Looked up in `tools/registry.py`'s provider map to select this Business's `ToolProvider` backend. Keyed by provider *type*, not `business_id` — a new Business onboards onto an existing type (e.g. `sqlite_menu`) with a yaml-only change. For `sqlite_menu`, the actual table is resolved by scanning the Business's entity tables for one with `tool_linked="sqlite_menu"` (see `docs/adr/0008`), not by `storage.table`/`.columns` below. A genuinely new backend shape still needs a new `ToolProvider` class and one registry entry. Unknown type → `UnknownToolProviderError`. |
| `storage` | [`StorageConfig`](#storageconfig) | no | `backend`/`database_id`/`schema_identity` stub, `table`/`columns` legacy | `table`/`columns` are superseded by the generic entity/table system (owner-managed via the admin UI, not Business Config) — see `docs/adr/0008`. Left in the schema, not removed, since existing Business Config files still validate against them. See `docs/adr/0003` for the stub fields' design. |
...
## `StorageConfig` (`backend`/`database_id`/`schema_identity` stub, `table`/`columns` legacy)

| Field | Type | Default | Notes |
|---|---|---|---|
| `backend` | `"none"` \| `"postgres"` \| `"sqlite"` | `"none"` | Not read by code — every Business's `ConversationRuntime` always gets a `SqliteSessionStore` + `SqliteCustomerStore` (plus, for `kampuscrave`, an `EntityBackedMenuRepository`) regardless of this value. `"sqlite"` here is truth-in-config only, documenting that this Business's session/customer/tool data genuinely lives in SQLite today — not a runtime switch. Both shipped Businesses (`hotel`, `kampuscrave`) are `"sqlite"` as of the live-menu-tool change; `"none"` is left as the schema default for a not-yet-onboarded Business. |
...
| `table` | string \| null | `null` | **Legacy, unread.** Superseded by the generic entity/table system (`docs/adr/0008`) — a `sqlite_menu` Tool now resolves its table by scanning for `tool_linked="sqlite_menu"` among the Business's owner-managed entity tables, not this field. Left in the schema since existing Business Config files still validate against it. |
| `columns` | dict[string, string] \| null | `null` | **Legacy, unread** — same story as `table` above. |
